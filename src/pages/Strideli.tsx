...
import { strideliContent } from '../data/strideli';
...
const Strideli = () => {
    return (
        <div className="relative w-full overflow-x-hidden font-sans bg-slate-950">
                    ...
                    <div className="flex flex-col lg:flex-row items-center gap-16">
                        <div className="lg:w-1/2 text-left">
                            <motion.div
                                ...
                                className="inline-flex items-center px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-[10px] font-bold mb-6 uppercase tracking-widest"
                            >
                                {strideliContent.hero.badge}
                            </motion.div>
                            ...
                            >
                                Precision <span className="text-cyan-400">Asset</span> Intelligence
                            </motion.h1>
                            ...
                            >
                                {strideliContent.hero.description}
                            </motion.p>
                            ...
                            >
                                <Link to="/demo" className="px-8 py-3.5 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-semibold rounded-lg transition-all flex items-center group shadow-lg shadow-cyan-500/20">
                        ...
                        >
                            ...
                            {/* Decorative Elements */}
                            <div className="absolute -top-12 -right-12 w-64 h-64 bg-cyan-500/10 blur-[100px] rounded-full" />
            ...
            {/* Strategic Intelligence - Elegant Light */}
            <section className="py-32 relative z-10 bg-white">
                <div className="max-w-4xl mx-auto px-6 text-center">
                    <motion.div
                        ...
                        viewport={{ once: true }}
                    >
                        <h2 className="text-xs font-bold text-cyan-600 mb-8 uppercase tracking-[0.3em]">The Intelligence Layer</h2>
                        <p className="text-lg md:text-xl text-slate-800 leading-relaxed font-normal italic font-serif max-w-3xl mx-auto">
                            "{strideliContent.executiveSummary.content}"
                        </p>
                    </motion.div>
                </div>
            </section>
            ...
            <section className="py-24 relative z-10 bg-slate-900 overflow-hidden border-y border-white/5">
                <div className="absolute inset-0 opacity-30 bg-[radial-gradient(circle_at_center,rgba(6,182,212,0.05),transparent)]" />
                <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-12">
                        {strideliContent.stats.map((stat, idx) => (
                            <motion.div
                                key={idx}
                                initial={{ opacity: 0, y: 20 }}
                                whileInView={{ opacity: 1, y: 0 }}
                            ...
                            >
                                <div className="text-4xl md:text-5xl font-bold text-white mb-3 tracking-tighter group-hover:text-cyan-400 transition-colors uppercase tracking-tight">{stat.value}</div>
            ...
            <section className="py-32 relative z-10 bg-slate-50">
                <div className="max-w-6xl mx-auto px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {strideliContent.painPoints.map((point, idx) => (
                            <motion.div
                                key={idx}
                                initial={{ opacity: 0, y: 20 }}
                                whileInView={{ opacity: 1, y: 0 }}
                            ...
                            >
                                ...
                                <div className="pt-6 border-t border-slate-100">
                                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-2">Structural Risk</p>
            ...
            <section className="py-32 relative z-10 bg-slate-950">
                <div className="max-w-7xl mx-auto px-6 lg:px-8">
                    <div className="text-center mb-20">
                        <h2 className="text-3xl md:text-5xl font-bold text-white mb-6 tracking-tight">Industrial Strength</h2>
                        <p className="text-slate-400 max-w-2xl mx-auto font-light leading-relaxed">Telemetry infrastructure engineered for reliability in the most demanding environments.</p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-16">
                        {strideliContent.features.map((feature, idx) => (
                            <motion.div
                                key={idx}
                                initial={{ opacity: 0, y: 20 }}
                                whileInView={{ opacity: 1, y: 0 }}
                            ...
                            >
                                <div className="mb-6 inline-flex p-3 rounded-lg bg-cyan-500/5 text-cyan-400 group-hover:bg-cyan-500 group-hover:text-slate-950 transition-all duration-300">
                                ...
                                <ul className="space-y-3">
                                    {feature.details.map((detail, i) => (
                                        <li key={i} className="flex items-center text-xs text-slate-300 hover:text-cyan-400 transition-colors">
                                            <div className="w-1 h-1 bg-cyan-500 rounded-full mr-3 opacity-40" />
                                            {detail}
                                        </li>
                                    ))}
                                </ul>
            ...
            <section className="relative py-32 bg-cyan-600 overflow-hidden">
                <div className="absolute inset-0 bg-[url('/grid.svg')] opacity-10 mix-blend-overlay" />
                <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
                    <h2 className="text-4xl md:text-6xl font-black text-slate-900 mb-8 tracking-tighter">Protect your high-value assets.</h2>
                    ...
                    <p className="mt-12 text-slate-900/60 text-[10px] font-bold uppercase tracking-[0.3em]">Precision Engineering for Global Fleets</p>
                </div>
            </section>
        </div>
    );
